...
function changecopy(o){
	...
	function queren(a){
		...
		//获取填写内容
		var $tr = $(a).parents("tr");
		var $input1 = $tr.children("td:eq(2)").children("input");//获取文本框
	...
	function updateGtid(gtid,$select){
		...
		//第一步，写一个web服务方法
		//第二步，：在这里写ajax请求，用上面方法
		//第三步：在业务层接口扩展一个更新商品类别的方法
		//第四步：在业务层实现类实现相应方法
		//第五步：在数据层DAO
		$.ajax({
			...
			success:function(r){
				var j = eval(r);
				$select.blur(function() { //失去焦点
				    $select.parent().text($select.find("option[value='" + j.gtid + "']").text()); 
				    $select.remove().hide(); //使文本框消失
				   })
			}
		})
	}
